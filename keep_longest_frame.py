...
# a minimum-length argument. In the case of a tie for longest frame,
# it will output all instances of the longest length.
...
def build_output_handle(infile_path, min_pep_length):
	# This function builds the sub-suffix for the output handle
	# e.g., input.fasta -> input.bf100.fasta for a length cutoff of 100
...
def process_seq(seq_record):
	...
	# Go through the ORFs; capture the longest length
	for orf in orfs:
	...
	if reading_frame == 6:
		max_len = max([SixFrameDict[i][0] for i in SixFrameDict.keys()]) # this is the maximum length for the six frames.
...
def reset_6tr_dict():
	...
	# We'll reset our dictionaries for the six-frame translations here
	global SixFrameDict
...
# Reset our dictionary for the family of six-frame translations
reset_6tr_dict()
