...
# this script was reconfigured from d1.sum_by_group_and_pfam_FPKMx_all_data2.py to work with kofam data:
# d1.sum_by_group_and_pfam_FPKMx_all_data2.py > d1.sum_by_group_and_kofam_FPKMx.py
# 5/20/2019
# Ryan Groussman

# d1.sum_by_group_and_kofam_FPKMx.py
"""
# FOR d1.sum_by_group_and_kofam_FPKMx.py:
# compute FPKMx-normalized values as well for each time point
# (previous version only does summed standard-normalized counts)

# counts summed up based on knum and phylo_group (e.g., all counts for haptophyte ferritin)
...
# input csv structure:
tax_id,knum,phylo_group,num_contigs,S11C1_A_1800,S11C1_C_1800,S14C1_B_2200,S14C1_C_2200,S15C1_B_200,S15C1_C_200,
...
S06C1_A_600,S06C1_C_600,S07C1_A_1000,S07C1_B_1000,S08C1_B_1400,S08C1_C_1400,length
"""
...
def parse_all_dat_row(CountsDict, row):
	...
	if knum not in CountsDict[phylo_group].keys():
		...
		# {1:[],2:[]}
		CountsDict[phylo_group][knum] = {'clock_time':{2:{1:[],2:[]}, 6:{1:[],2:[]}, 10:{1:[],2:[]}, 14:{1:[],2:[]}, 18:{1:[],2:[]}, 22:{1:[],2:[]}},'abs_time':{0:{1:[],2:[]}, 4:{1:[],2:[]}, 8:{1:[],2:[]}, 12:{1:[],2:[]}, 16:{1:[],2:[]}, 20:{1:[],2:[]}, 24:{1:[],2:[]}, 28:{1:[],2:[]}, 32:{1:[],2:[]}, 36:{1:[],2:[]}, 40:{1:[],2:[]}, 44:{1:[],2:[]}, 48:{1:[],2:[]}, 52:{1:[],2:[]}, 56:{1:[],2:[]}, 60:{1:[],2:[]}, 64:{1:[],2:[]}, 68:{1:[],2:[]}, 72:{1:[],2:[]}, 76:{1:[],2:[]}, 80:{1:[],2:[]}, 84:{1:[],2:[]}, 88:{1:[],2:[]}, 92:{1:[],2:[]}, 92:{1:[],2:[]}, }}
	# now we can start adding in times:
	for sample_name in samples:
		clock_time,abs_time,rep = get_times(sample_name)
		...
		CountsDict[phylo_group][knum]['clock_time'][clock_time]
		CountsDict[phylo_group][knum]['clock_time'][clock_time][rep].append(FpKMx)
		CountsDict[phylo_group][knum]['abs_time'][abs_time][rep].append(FpKMx)
...
def compute_stats(CountsDict):
	...
	# now we'll run through the CountsDict and compute stats for each of the group/kofam collections:
	for phylo_group in CountsDict.keys():
		StatsDict[phylo_group] = {}
		# print knum_list
		for knum in CountsDict[phylo_group].keys():
			if knum == 'hi_group':
				continue
			StatsDict[phylo_group][knum] = {'clock_time':{2:{}, 6:{}, 10:{}, 14:{}, 18:{}, 22:{}},'abs_time':{0:{}, 4:{}, 8:{}, 12:{}, 16:{}, 20:{}, 24:{}, 28:{}, 32:{}, 36:{}, 40:{}, 44:{}, 48:{}, 52:{}, 56:{}, 60:{}, 64:{}, 68:{}, 72:{}, 76:{}, 80:{}, 84:{}, 88:{}, 92:{}, 92:{}, }}
			# clock time:
			for clock_time in CountsDict[phylo_group][knum]['clock_time']:
				#print clock_time
				StatsDict[phylo_group][knum]['clock_time'][clock_time]['sum'] = sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][1]) + sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][2])
				StatsDict[phylo_group][knum]['clock_time'][clock_time]['max'] = max(sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][1]),sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][2]))
				StatsDict[phylo_group][knum]['clock_time'][clock_time]['min'] = min(sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][1]),sum(CountsDict[phylo_group][knum]['clock_time'][clock_time][2]))
				StatsDict[phylo_group][knum]['clock_time'][clock_time]['mean'] = (StatsDict[phylo_group][knum]['clock_time'][clock_time]['sum'] / 2)

			for abs_time in CountsDict[phylo_group][knum]['abs_time']:
				StatsDict[phylo_group][knum]['abs_time'][abs_time]['sum'] = sum(CountsDict[phylo_group][knum]['abs_time'][abs_time][1] + CountsDict[phylo_group][knum]['abs_time'][abs_time][2])
				StatsDict[phylo_group][knum]['abs_time'][abs_time]['max'] = max(sum(CountsDict[phylo_group][knum]['abs_time'][abs_time][1]),sum(CountsDict[phylo_group][knum]['abs_time'][abs_time][2]))
				StatsDict[phylo_group][knum]['abs_time'][abs_time]['min'] = min(sum(CountsDict[phylo_group][knum]['abs_time'][abs_time][1]),sum(CountsDict[phylo_group][knum]['abs_time'][abs_time][2]))
				StatsDict[phylo_group][knum]['abs_time'][abs_time]['mean'] = (StatsDict[phylo_group][knum]['abs_time'][abs_time]['sum'] / 2)


def output_csv(ContigData, out_file_name):


	clock_time_csv = open((out_file_name + ".clock_time.FPKMx_all_data.csv"), 'w')
	header = ['phylo_group', 'knum', 'hi_group', 'time', 'mean', 'min','max','sum']
	...
	for phylo_group in CountsDict.keys():
		...
		for knum in CountsDict[phylo_group]:
			...
			#print knum
			for clock_time in CountsDict[phylo_group][knum]['clock_time']:
				out_fields = [phylo_group, knum, hi_group, str(clock_time)]
				for stat in ['mean', 'min','max','sum']:
					out_fields.append(str(StatsDict[phylo_group][knum]['clock_time'][clock_time][stat]))
				#print out_fields
				#print ",".join(out_fields)
				clock_time_csv.write(",".join(out_fields)+ "\n")

	# and for absolute time:
	abs_time_csv = open((out_file_name + ".abs_time.FPKMx_all_data.csv"), 'w')
	header = ['phylo_group', 'knum', 'hi_group', 'time', 'mean', 'min', 'max', 'sum']
	...
	for phylo_group in CountsDict.keys():
		...
		for knum in CountsDict[phylo_group]:
			if knum == 'hi_group':
				continue
			for abs_time in CountsDict[phylo_group][knum]['abs_time']:
				out_fields = [phylo_group, knum, hi_group, str(abs_time)]
				for stat in ['mean', 'min', 'max', 'sum']:
					out_fields.append(str(StatsDict[phylo_group][knum]['abs_time'][abs_time][stat]))
...
# step 1: load in an 'd1.kofam_counts_by_taxid.csv' file:
